function [grad_z,grad_x,k,sws_matrix] = phase_estimator_TotalVariationPDO(u, w_kernel,f_v,dinf,og_size,constant,pars)
% function [grad_z,grad_x,k,sws_matrix] = phase_estimator_TotalVariationPDO(u, w_kernel,f_v,dinf,og_size,constant,pars)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    for ii = 1:og_size(1)
        for jj = 1:og_size(2) %% for faster computing pararell toolbox
            ...
            %% QR solver
            b = area(:);
            [results, cost, error, fide, regul] = pdo_TVV_single(area, A, lambda, tau, maxIter, tol, numberEstimators, stableIter)
            results_vec = results(:);
			
            grad_z(ii,jj) = results_vec(2);
        end
    end
