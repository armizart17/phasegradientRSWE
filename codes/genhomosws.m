...
for freq = v_freq
    ...
    pathfreq_in = [pathdata,'Data', num2str(freq),'Hz-',num2str(nWaves),'wvs/'];
...
%         name = ['R-FIELD_homo_',num2str(field),'.mat'];% homo mantain
        ...
        dinf.dx = min(diff(R_Field.x));
        dinf.dz = min(diff(R_Field.z));
        frame = R_Field.pv_complexZ(:,:,1) + sigmaNoise * randn(size(frame));% number of frame
...
t = toc;
fprintf('QR v1.0 Kernel time : %.2f seconds\n', t);
...
window = 1; %11 pixels as described in paper
window = 15;
...
t = toc;
fprintf('QR v2.0 BigMat time : %.2f seconds\n', t);
...
fprintf('Tikho v%.1f time : %.2f seconds\n', parsTK.version, t)
...
% imagesc(R_Field.x*cm, R_Field.z*cm, pg_TKv1.sws_matrix_big);
colormap(gca, jet); % Apply jet colormap to the current axes
colorbar;
axis equal; axis tight;
xlabel('Lateral'), ylabel('Axial'), 
...
%% (NOT YET)
% GENERATE DATA SWS INCLUSION ONE FRAME WITH PHASE ESTIMATOR QR SOLVER v2.2 BIG MATRIX (NOT YET)

addpath(genpath(pwd));
fprintf('-------QR solver 2.2-------\n')
...
window = 3; %11 pixels as described in paper
window = 15;
...
for freq = v_freq
    ...
    for field = 1:nFields
        ...
        % FUNCION SWS MAP (Con linearizacion)
        [grad_z,grad_x,k,sws_matrix] = phase_estimator_QR_bigmat_v2(mirror_frame, w_kernel, freq, dinf, og_size, 1.6, stride);
    
        % EMPAQUETAR RESULTADOS
        
        pg_QRv2_2.grad_z = grad_z;
        pg_QRv2_2.grad_x = grad_x;
        pg_QRv2_2.grad_k = k;
        pg_QRv2_2.sws_matrix = sws_matrix;
       
        
    % Save
    pathQR = [pathfreq_out, 'PhaseGradientQRv2_2/'];
    if ~exist(pathQR,"dir"); mkdir(pathQR); end
%     save([pathQR, 'SWS_PG_QRv2_2_homo_',num2str(field),'.mat'],'pg_QRv2_2');
...
subplot(122),
% imagesc(R_Field.x*cm, R_Field.z*cm, pg_QRv2.sws_matrix, [0 5]);
imagesc(R_Field.x*cm, R_Field.z*cm, pg_QRv2.sws_matrix);
colormap(gca, jet); % Apply jet colormap to the current axes
colorbar;
axis equal; axis tight;
xlabel('Lateral'), ylabel('Axial'), 
